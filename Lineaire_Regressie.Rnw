...
Met deze waarden van $b_{0}$ en $b_{1}$ kunnen de berekende waarden en de fouten bepaald worden:
\begin{equation}
  \begin{split}
...
@
\medskip
De som van de afwijkingen ($\sum_{i=1}^{i=n}u_{i}=0$) en de afwijkingen en de onafhankelijke variabele zijn ortogonaal ($\sum_{i=1}^{i=n}u_{i}x_{i}=0$).
...
We kunnen bewijzen\sidenote{''Kwantitatieve beleidsmethoden: enkelvoudige en meervoudige regressie", Peter Goos, 2014, pp. 14 e.v.} dat $b_{0}$ en $b_{1}$ onvertekende schatters zijn van de Platonische $\beta_{0}$ en $\beta_{1}$:
\begin{equation}
  \begin{split}
  E(B_{0})&=\beta_{0} \\
  E(B_{1})&=\beta_{1}
...
<<echo=FALSE>>=
sse <- sum(geg$u^2)
s_sq <- round(sse/(n-2),2)
@

In dit voorbeeld is $s^{2}=$\Sexpr{s_sq}. En dus zijn de schatters voor de variantie van $B_{0}$, $B_{1}$ en hun covariantie gelijk aan:
...
\begin{equation}
  \begin{split}
  \frac{B_{0} - \beta_{0}}{\frac{\sigma}{\sqrt{SS_{xx}}}\sqrt{\frac{\sum\limits_{i=1}^{i=n}x_{i}^{2}}{n})}}  &\sim T_{(n-2)}  \\
  \frac{B_{1} - \beta_{1}}{\frac{\sigma}{\sqrt{SS_{xx}}}} &\sim T_{n-2}
  \end{split}
\end{equation}
