...
sigma_U <- 2
set.seed(2018)
intro2 <- data.frame(x = runif(N, 1, 5), y=0)
intro2$y <- b0 + b1*intro2$x + rnorm(N, 0, sigma_U)
ggplot(data=intro2) +
...
sigma_U <- 0.5
set.seed(2018)
intro1 <- data.frame(expnr = rep((1:N), each=n), x = rep(seq(1,n),N), y = 0)
intro1$y <- b0 + b1*intro1$x + rnorm(n*N, 0, sigma_U)
ggplot(data=intro1) +
...
\newpage
\newthought{Samenvatting van alle diagnostische plots} (Figuur~\ref{fig:diagnostic_plots}):
