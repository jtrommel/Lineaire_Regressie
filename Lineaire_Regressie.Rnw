...
Het onderscheid tussen een \emph{functioneel} en een \emph{statistisch} verband is dat de waarnemingen bij een statistisch verband \emph{niet} gelegen zijn op de grafiek, maar in de buurt daarvan (Figuur~\ref{fig:intro1}).
...
ggplot(data=intro1) +
  ...
  geom_abline(intercept=-1, slope=7, color="red") +
  xlim(0, 6) +
...
De waarneming $Y$ is een kansveranderlijke veroorzaakt door de storende factoren en gegroepeerd in de kansveranderlijke U:
\begin{equation}
...
Daarvoor berekenen we de parti\"ele afgeleiden van $Q$ naar $\beta_{0}$ en $\beta_{1}$ en stellen die gelijk aan nul. Uit de twee vergelijkingen volgen twee getalwaarden $b_{0}$ en $b_{1}$ die berekend worden op basis van de steekproefgegevens. Hierbij worden wat tussenresultaten berekend. 
\begin{equation}
  \begin{split}
...
\end{center}
\medskip
Met deze waarden van $b_{0}$ en $b_{1}$ kunnen de berekende waarden en het verschil tussen de berekende ($\hat{y}_{i}$) en waargenomen waarde ($y_{i}$) bepaald worden. Dat verschil noemen we het \emph{residu} ($r_{i}$):
\begin{equation}
r_{i}=y_{i}-\hat{y}_{i}=y_{i}-b_{0}-b_{1}x_{i}
\end{equation}

Het residu $r_{i}$ is dus niet enkel afhankelijk van de waarneming die verstoord wordt door de storingen gegroepeerd in $U_{i}$, maar ook door de waarden van de steekproef die $b_{0}$ en $b_{1}$ bepalen. De variantie van $r_{i}$ zal dus groter zijn dan de variantie van de foutterm $U_{i}$. We maken daarom een duidelijk onderscheid tussen de foutterm $U(x)$ (die niet toegankelijk is) en de residus $r_{i}$ (die, voor elke steekproef, berekenbaar zijn). Bovendien zal de variantie van $r_{i}$ \emph{verschillend} zijn van de variantie van $r_{j}$ wanneer $i \neq j$! Zie hiervoor \ref{subsec:Veronderstelling3}.
...
\begin{itemize}
  ...
  \item de residus $r_{i}$ en de onafhankelijke waarden $x_{i}$ zijn ortogonaal. Omslachtig bewijs\sidenote{''Kwantitatieve beleidsmethoden: enkelvoudige en meervoudige regressie", Peter Goos, 2014, pp. 13}
...
\begin{equation}
SSE=\sum\limits_{i=1}^{i=n}\hat{R_{i}}^{2}=\sum\limits_{i=1}^{i=n}(Y_{i}-B_{0}-B_{1}x_{i})^{2}
\end{equation}
...
Behalve wanneer er herhalingsmetingen zijn, kennen we geen meerdere waarden voor $y_{i}$, en dus ook geen meerdere waarden voor $r_{i}$. We kunnen dus niet nagaan of $E(U_{i})=0$.  
...
simul1$y <- b0 + b1*simul1$x + rnorm(nsim*Nsim, 0, sigma_U)
...
\newthought{De hat-matrix H} komt naar voor als we een matrixnotatie gebruiken voor de residus. Dat maakt uitbreiding naar lineaire modellen met m\'e\'er dan \'e\'en onafhankelijke variabele zeer gemakkelijk. In formulevorm zijn de residus gegeven door:
\begin{equation}
...
Indien elke waarneming hetzelfde belang zou hebben dan zouden de diagonaalwaarden overal gelijk moeten zijn aan het aantal modelparameters gedeeld door $n$. In dit geval is dat \Sexpr{round(sum(diag(H))/n,3)}. Wanneer we de matrix $\boldsymbol{H}$ bekijken dat is dat niet het geval: \emph{de eindpunten} hebben een hoger dan gemiddelde hefboom, \emph{de middelpunten} hebben een lager dan gemiddelde hefboom.
...
xmin <- min(linmod$.fitted) - 0.1*(abs(min(linmod$.fitted)))
xmax <- max(linmod$.fitted) + 0.1*(abs(max(linmod$.fitted)))
ymin <- min(c(-3,min(linmod$.std.resid) - 0.1*(abs(min(linmod$.std.resid)))))
ymax <- max(c(3,max(linmod$.std.resid) + 0.1*(abs(max(linmod$.std.resid)))))
p1 <- ggplot(data=linmod, aes(x = .fitted, y = .std.resid)) +
  geom_point() +
  geom_label(label=rownames(linmod), nudge_x = 0.3, nudge_y = 0.1, size=2.5) +
...
Omdat het teken van de residutermen eigenlijk niet belangrijk is, gebruikt men ook de ''Scale-Location''-plot waarbij men de wortel van de absolute waarde van de residus $|r_{i}|$ uitzet in functie van de berekende waarde $y_{i}$ (Figuur~\ref{fig:scale_location}). Men neemt de vierkantswortel van de absolute waarden van de residus omdat de scheefheid daarvan kleiner is dan de scheefheid van de absolute waarde op zich. Die waarden zouden op een horizontale lijn moeten liggen (niet de nullijn).

<<label=scale_location,fig=TRUE,include=FALSE, echo=FALSE>>=
#  lm_abs_stud_res <- lm(abs(student_res) ~ yhat, data=geg)
ymin <- -0.1*max(sqrt(abs(linmod$.std.resid))) 
ymax <- 1.1*max(sqrt(abs(linmod$.std.resid))) 
p3 <- ggplot(data=linmod, aes(x = .fitted, y = sqrt(abs(.std.resid)))) +
  geom_point() +
  geom_label(label=rownames(linmod), nudge_x = 0, nudge_y = 0.75*abs(ymin), size=2.5) +
  xlim(xmin, xmax) +
  ylim(ymin, ymax) +
  geom_hline(yintercept=0, col="blue") +
...
xmin <- min(kwantiel$q1) - 0.1*(abs(min(kwantiel$q1)))
xmax <- max(kwantiel$q1) + 0.1*(abs(max(kwantiel$q1)))
ymin <- min(kwantiel$q2) - 0.1*(abs(min(kwantiel$q2)))
ymax <- max(kwantiel$q2) + 0.1*(abs(max(kwantiel$q2)))
p2 <- ggplot(kwantiel, aes(q1, q2)) +
  geom_point(na.rm = TRUE) +
  geom_label(label=rownames(kwantiel), nudge_x = 0, nudge_y = 0.1*(abs(max(kwantiel$q2))), size=2.5) +
  geom_line(aes(x=q2, y=q2)) +
  # geom_line(aes(q2, q2), col="red", linetype="dashed") +
  xlim(xmin, xmax) +
  ylim(ymin, ymax) +
  xlab("Theor. Studentized Quantielen") +
  ylab("Waarg. Studentized Quantielen") +
...
De berekening van Cook's Distance lijkt ingewikkeld omdat men telkens een bijkomend lineair model moet opstellen waarbij de i-de waarneming wordt uitgesloten. Gelukkig kan men dit herleiden tot een eenvoudigere berekening waarbij we enkel de diagonaalelementen van de hat-matrix $H$ nodig hebben:
\begin{equation}
...
De kans dat we deze Cook's Distances vinden wordt berekend met de $F_{m,(n-m)}$-kansverdeling:
\begin{equation}
...
Hier zijn alle punten invloedrijk, maar dat komt omdat het aantal punten $n$ zeer klein is (5).
...
p6 <- ggplot(linmod, aes(.hat/(1-.hat), .cooksd)) +
  geom_point(na.rm=TRUE) +
  geom_label(label=rownames(linmod), nudge_x = 0.1, nudge_y = 0.01, size=2.5) +
  xlab("hii/(1-hii)") +
  ylab("Cook's Distance") +
...
Aan de linkerkant zien we dat Cook's Distance (grootte van het meetpunt) groter wordt naarmate de leverage $h_{ii}$ en het residu $r_{i}$ groter worden. Aan de rechterkant zien we de lineaire relatie tussen Cook's Distance en de breuk $\frac{h_{ii}}{(1-h_{ii})}$. De richtingsco\"effici\"ent van de rechte wordt bepaald door het kwadraat van het residu $r_{i}$. De stippellijnen geven rechten met richtingsco\"effici\"enten gaande van 0 tot 3 in stappen van 0.5. Beide figuren zeggen hetzelfde: meetpunten 1 en 5 (de uitersten) zijn de meest invloedrijke.
