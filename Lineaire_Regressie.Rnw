...
p1 <- ggplot(data=linmod, aes(x = .fitted, y = .std.resid)) +
  geom_point() +
  geom_label(label=rownames(linmod), nudge_x = 0.2*(abs(min(linmod$.fitted))), nudge_y = 0, size=2.5) +
...
p3 <- ggplot(data=linmod, aes(x = .fitted, y = sqrt(abs(.std.resid)))) +
  geom_point() +
  geom_label(label=rownames(linmod), nudge_x = 0, nudge_y = 0.1*max(sqrt(abs(linmod$.std.resid))), size=2.5) +
  xlim(xmin, xmax) +
  ylim(ymin, ymax) +
  geom_hline(yintercept=0, col="blue") +
...
xmax <- max(kwantiel$q1) + 0.2*(abs(max(kwantiel$q1)))
ymin <- min(kwantiel$q2) - 0.2*(abs(min(kwantiel$q2)))
ymax <- max(kwantiel$q2) + 0.2*(abs(max(kwantiel$q2)))
p2 <- ggplot(kwantiel, aes(q1, q2)) +
  geom_point(na.rm = TRUE, size=1) +
  geom_label(label=rownames(kwantiel), nudge_x = 0.1*(abs(max(kwantiel$q2))), nudge_y = 0, size=2.5) +
...
p5 <- ggplot(linmod, aes(.hat, .std.resid)) +
  ...
  geom_label(label=rownames(linmod), nudge_x = 0.2*(abs(min(linmod$.hat))), nudge_y = 0, size=2.5) +
  xlim(xmin,xmax) +
  ylim(ymin,ymax) +
  xlab("Leverage hii") +
...
p6 <- ggplot(linmod, aes(.hat/(1-.hat), .cooksd)) +
  ...
  geom_label(label=rownames(linmod), nudge_x = 0.2*(abs(min(linmod$.hat/(1-linmod$.hat)))), nudge_y = 0.01, size=2.5) +
  xlab("hii/(1-hii)") +
  ylab("Cook's Distance") +
