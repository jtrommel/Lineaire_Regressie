...
\begin{figure}
\centering
\includegraphics[height=7cm]{LinReg-controle_veronderstelling3}
\caption{ }
...
ggplot(data=geg) +
  geom_point(aes(x = yhat, y = student_res)) +
...
ggplot(data=geg, aes(x = yhat, y = sqrt(abs(student_res)))) +
  geom_point() +
  geom_smooth(col="red") +
  xlim(0, 5) +
  ylim(-1, 3) +
...
quantile1 <- qqnorm(linmod$.std.resid, plot.it = FALSE)[[1]]
quantile2 <- qqnorm(linmod$.std.resid, plot.it = FALSE)[[2]]
ggplot(linmod, aes(quantile1, quantile2)) +
  geom_point(na.rm = TRUE) +
  geom_line(aes(quantile2, quantile2), col="red", linetype="dashed") +
...
\subsection{Andere elementen om in de gaten te houden}
...
ggplot(linmod, aes(seq_along(.cooksd), .cooksd)) +
...
p2 <- ggplot(linmod, aes(quantile1, quantile2)) +
  geom_point(na.rm = TRUE) +
  geom_line(aes(quantile2, quantile2), col="red", linetype="dashed") +
  xlab("Theor. Studentized Quantielen") +
  ylab("Waarg. Studentized Quantielen") +
...
p3 <- ggplot(linmod, aes(.fitted, abs(.std.resid))) +
  geom_point(na.rm=TRUE) +
  xlab("Berekende waarden") +
  ylab("|Gestandaardiseerde residuals|") +
  ggtitle("Scale-Location") +
  JT.theme
    
p4 <- ggplot(linmod, aes(seq_along(.cooksd), .cooksd)) +
  geom_bar(stat="identity", position="identity") +
  xlab("Obs. Number") +
  ylab("Cook's distance") +
  ggtitle("Cook's distance") +
...
p5 <- ggplot(linmod, aes(.hat, .std.resid)) +
  ...
  xlab("Invloed (leverage)") +
  ylab("Gestandaardiseerde Residus") +
  ggtitle("Invloedrijke punten", subtitle="Residual vs Leverage Plot") +
...
\caption{Samenvatting van alle diagnostische plots}
