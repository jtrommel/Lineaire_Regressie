...
Omdat het teken van de residutermen eigenlijk niet belangrijk is, gebruikt men ook de ''Scale-Location''-plot waarbij men de absolute waarde van de residus $|r_{i}|$ uitzet in functie van de berekende waarde $y_{i}$ (Figuur~\ref{fig:scale_location}). Die waarden zouden op een horizontale lijn moeten liggen (niet de nullijn).
