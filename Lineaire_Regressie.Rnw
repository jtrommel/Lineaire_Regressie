...
Met vergelijking (\ref{eq:stud.resid}) kunnen we Cook's Distance schrijven i.f.v. de studentized residus:
\begin{equation}
D_{i}=\frac{1}{m}studres_{i}^{2}\left[ \frac{h_{ii}}{\left( 1 - h_{ii} \right)} \right]
\end{equation}
...
Aan de linkerkant zien we dat Cook's Distance (oppervlakte van het punt) groter wordt naarmate de leverage $h_{ii}$ en het residu $r_{i}$ groter worden. Aan de rechterkant zien we de lineaire relatie tussen Cook's Distance en $\frac{h_{ii}}{1-h_{ii}}$. De richtingsco\"effici\"ent van de rechte wordt  bepaald door het kwadraat van het studentized residu $studres_{i}$. De stippellijnen geven rechten met richtingsco\"effici\"enten gaande van 0 tot 3 in stappen van 0.5. Beide figuren zeggen hetzelfde: meetpunten 1 en 5 (de uitersten) zijn de meest invloedrijke. In de linker- en middenfiguur is de grens $h_{ii}=2\frac{m}{n}$ aangegeven die wijst op grote leverage. In de rechterfiguur is die grens $\frac{2m}{n-2m}$.
...
\section{Eigen lineaire regressie functie}

De hele berekening is gecentreerd op de ''hat-matrix'' van vergelijking \ref{eq:hatmatrix}
\begin{equation}
\boldsymbol{H}=\boldsymbol{X}\left( \boldsymbol{X'}\boldsymbol{X} \right)^{-1}\boldsymbol{X'}
\end{equation}

waarmee we de parameters $\beta$ kunnen berekenen:
\begin{equation}
  \boldsymbol{b}=\left( \boldsymbol{X'}\boldsymbol{X} \right)^{-1}\boldsymbol{X'}\boldsymbol{Y}
\end{equation}

We gieten dit in een eigen functie, wat een aangepaste versie is van wat verscheen in R-bloggers\sidenote{Create your Machine Learning Library from scratch with R! (1/5) - Linear and Logistic Regression (30.01.2018)}

<<>>=
JTR_fit_lm <- function(x, y, intercept=TRUE, lambda=0) {
  # checking it the input of independent variables is in the form of a matrix. If not, convert to matrix
  if (!is.matrix(x)) {
    x <- as.matrix(x)
  }
  # checking it the input of response variable is in the form of a matrix. If not, convert to matrix
  if (!is.matrix(y)) {
    y <- as.matrix(y)
  }
  # if an intercept, different from 0, is wanted (the default) we add a column with 1's to the x matrix
  if (intercept) {
    x <- cbind(1,x) # changed cbind(x,1) to cbind(1,x) to get b0 as the first coëfficiënt, and b1 as the second.
  }
  my_lm <- list(intercept=intercept)
  my_lm[['coeffs']] <- solve(t(x) %*% x) %*% t(x) %*% y # calculates the coefficients
  my_lm[['yhat']] <- x %*% my_lm[['coeffs']] # calculates the yhat values. Changed from 'preds' to 'yhat'
  my_lm[['residuals']] <- my_lm[['yhat']] - y # calculates the residuals
  my_lm[['mse']] <- sum(my_lm[['residuals']]^2)/(dim(x)[1]-2) # changed from mean(my_lim[['residuals]]^2)
...
<<>>=
JTR_pred_lm <- function(my_lm, x , ..) {
  if (!is.matrix(x)) {
    x <- as.matrix(x)
  }
  if (my_lm[['intercept']]) {
    x <- cbind(1,x) # changed from cbind(x,1)
  }
  pred_lm <- cbind(x,x %*% my_lm[['coeffs']])[,2:3] # changed from x %*% my_lm[['coeffs]] to get a result with x- and y- values
  return(pred_lm) # added
}
@

<<>>=
JTR_plot_lm <- function(my_lim, bins=30, ..) {
  ggplot(data=as.data.frame(my_lim[['residuals']]), aes(x=V1)) + 
    geom_histogram(bins=bins) +
    labs(title = "Histogram van de residus", 
       x = "residus ri", 
       y = "aantal")
}
@
