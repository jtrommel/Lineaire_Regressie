...
x1 <- as.numeric(quantile(kwantiel$q1)[2])
x2 <- as.numeric(quantile(kwantiel$q1)[4])
y1 <- as.numeric(quantile(kwantiel$q2)[2])
y2 <- as.numeric(quantile(kwantiel$q2)[4])
slope <- (y2-y1)/(x2-x1)
interc <- y1 - x1*slope
...
p4 <- ggplot(linmod, aes(seq_along(.cooksd))) +
  geom_linerange(aes(ymin = rep(0,n), ymax = .cooksd), size=1.5) +
